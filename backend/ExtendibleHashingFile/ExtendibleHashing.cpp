...
void ExtendibleHashing<RECORD, TK>::loadCSV(const string& filename) {
  ...
  while (std::getline(file, line)) {
    ...
    if constexpr (std::is_same_v<RECORD, EHRecordA>) {
      ...
      record = EHRecordA(id, name, album, album_id, artists);
    } 
    else if constexpr (std::is_same_v<RECORD, EHRecordB>) {
      int year;
      char make[30], model[50], vin[18];
      ...
      record = EHRecordB(year, make, model, vin);
    }
...
template <class RECORD, class TK>
bool ExtendibleHashing<RECORD, TK>::add(const RECORD& reg) {
