...
class SequentialFileB {
    ...
    void merge();
    void insert_aux(SeqRecordB record);
...
public:
    ...
    int total_records();    
    void insert(SeqRecordB record);
    void remove_record(string key);
    vector<SeqRecordB> range_search(string begin_key, string end_key);
    SeqRecordB search(string key);
    void show_records(string name, int max_records);
    void destroy();
    void description();
    void create_file(vector<SeqRecordB>&records);
};
...
void SequentialFileB::insert_aux(SeqRecordB record){
    File_data.open(aux_data, ios::in | ios::out | ios::binary);
...
void SequentialFileB::insert(SeqRecordB record){
...
void SequentialFileB::remove_record(string key) {
    ...
    if (flag) {
        ...
        File_data.open(aux_data, ios::in | ios::out | ios::binary);
        for (SeqRecordB record : aux_records) {
            File_data.write((char*) &record, Record_size);
        }
        File_data.close();
        return;
    ...
    vector<SeqRecordB> data_records;
    File_data.open(filename_data, ios::in | ios::out | ios::binary);
    File_data.seekg(0, ios::beg);
    while (File_data.read((char*) &record, Record_size)) {
        if (record.key() == key){
    ...
    File_data.open(filename_data, ios::in | ios::out | ios::binary);
    for (SeqRecordB record : data_records) {
        File_data.write((char*) &record, Record_size);
    }
    File_data.close();
...
auto compareB = [](SeqRecordB a, SeqRecordB b) -> bool {
    return a.key() < b.key();
};
...
vector<SeqRecordB> SequentialFileB::range_search(string begin_key, string end_key) {
    vector<SeqRecordB> records;
    SeqRecordB record;
...
SeqRecordB SequentialFileB::search(string key){
...
vector<SeqRecordB> read_from_csvB(string filename){
    vector<SeqRecordB> records;
...
void SequentialFileB::create_file(vector<SeqRecordB>& records){
    sort(records.begin(), records.end(), compareB);

    File_data.open(filename_data, ios::in | ios::out | ios::binary);
    File_data.seekp(0, ios::beg);

    for (SeqRecordB record : records) {
        File_data.write((char*) &record, Record_size);
    }
